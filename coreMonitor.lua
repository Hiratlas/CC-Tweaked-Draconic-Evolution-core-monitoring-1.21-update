...
_G.MonWidth, _G.MonHeight = mon.getSize()
if MonWidth <= 29 then
    _G.TextScale = 1
else
    _G.TextScale = 1
...
_G.Scale = 1 -- TODO Element scaling outside of the default resolution mode. DON'T CHANGE CURRENTLY.
    ...
    elseif currentControls == "editInput" or currentControls == "editOutput" then
        ...
        if putLimit ~= "" then
            if tonumber(putLimit) <= 2147483647 then
                ...
                mon.write(putLimit)
                _G.putLimitNum = tonumber(putLimit)
                mon.setBackgroundColor(colors.black)
                _G.fix = 0
...
local function drawDetails(xPos, yPos)
    ...
    if LimitTransfer == true then
        _G.inputRate = input.getFlow()
        _G.outputRate = output.getFlow()
    end
...
local function drawAll()
    while true do
        local versionText = "Version " .. Version .. " by AzireVG updated by Hiratlas"
